...
휴대폰 알림 기능을 사용자가 직접 활성화한 경우에만 `ntfy.sh`로 알림 전달에 필요한 정보를 전송할 수 있습니다. 자세한 내용은 8절을 참고하세요.
...
- ntfy Topic은 충분히 긴 무작위 값으로 생성하며 사용자가 직접 재생성할 수 있습니다.
...
사용자가 휴대폰 알림을 활성화한 경우 Chrome에서 감지한 Velog 알림을 휴대폰으로 전달하기 위해 사용합니다.

## 8. 휴대폰 알림 및 ntfy

휴대폰 알림 기능은 선택 기능이며 기본값은 OFF입니다.

사용자가 기능을 활성화하면 다음 정보가 `https://ntfy.sh`로 전송될 수 있습니다.
...
기본 방식에서는 무작위 Topic 이름 자체가 접근 비밀값 역할을 합니다. Topic을 공개하지 않아야 하며, 노출이 의심되는 경우 새 Topic을 생성해야 합니다.
